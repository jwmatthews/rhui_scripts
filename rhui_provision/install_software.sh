...
ansible-playbook install_v2_software.yml -i ${ANSIBLE_INVENTORY} -vvvv --private-key=${EC2_SSH_PRIV_KEY} --extra-vars "iso_path=$ISO_PATH rhui_iso_filename=$ISO_FILENAME region=$REGION" | tee ${LOG_DIR}/install_software.log
