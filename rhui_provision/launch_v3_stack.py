...
def _create_pulp_part(ssh_client, blockdevice, vgname, lvname, mountpoint):
    # RHUI v3 setup
    cmd = "sudo pvcreate %s" % (blockdevice)
    run_cmd(ssh_client, cmd)
    ...
    cmd = "sudo /sbin/mkfs.xfs -f -i size=512 /dev/%s/%s" % (vgname, lvname)
    run_cmd(ssh_client, cmd)

    cmd = "sudo mkdir -p /export/rhui_content"
    run_cmd(ssh_client, cmd)

    cmd = "sudo mount /dev/%s/%s /export/rhui_content" % (vgname, lvname)
    run_cmd(ssh_client, cmd)

    cmd = "sudo mkdir -p /export/rhui_content/brick"
    run_cmd(ssh_client, cmd)
...
def setup_filesystem_on_host(instance_details, ssh_user, ssh_priv_key_path):
    ...
    if "HAPROXY" not in instance_details["tags"]["Role"]: # don't need to do this on haproxy
    	logging.info("Setting up %s on '%s'" % (pulp_mountpoint, dns_name))
    	_create_pulp_part(ssh_client, blockdevice="/dev/xvdp", 
