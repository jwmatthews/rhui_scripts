...
./launch_v2_stack.py --template ${CLOUD_FORMATION_TEMPLATE} --vpc_id ${RHUI_VPC_ID} --subnet_id ${RHUI_SUBNET_ID} --bash_out_file ${HOSTNAMES_ENV} --ans_out_file ${ANSIBLE_INVENTORY} --ssh_user ${EC2_SSH_USER} --ssh_key_name ${EC2_SSH_KEY_NAME} --ssh_priv_key_path ${EC2_SSH_PRIV_KEY} --region ${REGION} --instance_type ${DEFAULT_EC2_INSTANCE_TYPE} | tee /tmp/launch_v2_stack.log
if [ "$?" -ne "0" ]; then
